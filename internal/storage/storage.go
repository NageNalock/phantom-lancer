...
// DatabaseStats returns only the database file size. Per-table row counting was
// intentionally removed because polling it caused full-table reads on large
...
func (s *Store) DatabaseStats(_ context.Context) (DatabaseStats, error) {
	totalBytes, err := s.DatabaseSizeBytes()
	if err != nil {
		return DatabaseStats{}, fmt.Errorf("get db file size: %w", err)
	}
	return DatabaseStats{TotalBytes: totalBytes, UpdatedAt: now()}, nil
}
...
func (s *Store) DeleteImageGenerationJob(ctx context.Context, id string) error {
	id = strings.TrimSpace(id)
	if id == "" {
		return ErrNotFound
	}
	...
	res, err := tx.ExecContext(ctx, `DELETE FROM image_generation_jobs WHERE id = ?`, id)
	if err != nil {
		return err
	}
