...
func (s *Service) mcpSubmitResult(args json.RawMessage) (any, *mcpError) {
	...
	entry, ok := s.agentTaskPool.getTask(params.TaskID)
	if !ok {
		return nil, &mcpError{Code: mcpErrInvalidParams, Message: ErrTaskNotFound.Error(), Data: map[string]string{"status": "invalid_task"}}
	}
	...
	if params.TaskType == AgentTaskTypeOpportunityDiscovery {
		run, lookupErr := s.store.GetOpportunityDiscoveryRunByAgentRunID(contextFromMCP(), entry.agentRunID)
		if lookupErr != nil {
			return nil, mcpErrorFromError(lookupErr)
		}
		if _, processErr := s.ProcessOpportunityDiscoverySubmittedResult(contextFromMCP(), run.ID, result); processErr != nil {
			return nil, mcpErrorFromError(processErr)
		}
	}
