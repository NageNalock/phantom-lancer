...
func buildOpportunityDiscoveryPrompt(taskID string, discCtx OpportunityDiscoveryContext, mcpURL string) string {
	...
	b.WriteString("Before finalizing any company as a candidate, locate and use its latest available periodic report as of this run's BuiltAt time (latest annual report plus a newer interim or quarterly report when available). Verify the reporting period and publication date. Older reports may be used only as historical comparison and must not be the sole company evidence when a newer periodic report exists. Record material changes from the older evidence. If the latest report cannot be verified, mark that candidate's evidence freshness degraded and lower its evidence score and confidence.\n")
	if discCtx.Mode != OpportunityDiscoveryModeMarketScan {
...
func buildPortfolioSentinelPrompt(taskID string, pack PortfolioSentinelContext, mcpURL string) string {
	...
	b.WriteString("The supplied context is the primary evidence pack. Do not repeat an equivalent query for facts already present and usable. Use MCP for mandatory pagination and for a specific missing, conflicting, or structurally compressed fact; batch symbols when supported. A daily-bar summary is not a substitute for raw OHLCV when evaluating accumulation/distribution.\n")
	...
	b.WriteString("Available internal evidence tools include stock_agent.get_daily_bars, get_minute_bars, get_quote_history, get_fund_flow_history, get_decision_evidence, and get_market_scan_candidates, in addition to profiles, news threads, strategies, and portfolio context. Choose only the smallest relevant calls; never infer that data is unavailable before calling the applicable tool.\n")
	...
	b.WriteString("1c. Inspect each decisionGates.market_structure result and its potentialSupplyPressure, postBreakoutDistribution, highVolumeStall, close-location, and recent-volume metrics. When it is blocked/degraded or conflicts with the narrative, call stock_agent.get_daily_bars for the raw sequence before deciding; use get_minute_bars or get_quote_history for current-session confirmation and get_fund_flow_history for flow divergence. Treat high turnover with weak price progress, repeated low closes, failed breakouts, and gap fades as possible supply/distribution risk, not as bullish volume by default.\n")
	...
	b.WriteString("4. Review every holding. trustedCandidates is a bounded recent opportunity handoff, not a second full-market assignment: use its supplied evidence first and perform additional research only when one candidate is strong enough to justify build_position. A non-held symbol may appear only as build_position and only when it is present in trustedCandidates.\n")
	...
	b.WriteString("5. Before build_position or add_position, or whenever an action relies on a news/fundamental causal claim, perform real public retrieval using Codex web_search or a named search/research/browse Agent tool, record compact source/claim metadata in research_audit, and reference those IDs from the action. A defensive reduce/exit plan may instead rely on fresh internal quote/bar/flow evidence: include the exact decisionGates snapshot id in evidence_refs and state that no external cause was assumed.\n")
	...
	b.WriteString("`research_audit` records compact real retrieval evidence with id, kind, query, source, source_title, published_at/checked_at, and claim. Build/add and externally-caused actions need non-empty research_refs pointing to these IDs. A defensive reduce/exit without research_refs must include the matching decisionGates snapshot id in evidence_refs. Never claim a search that was not actually performed.\n\n")
	b.WriteString("Example submit_result shape:\n")
	b.WriteString("```json\n")
	coverageExample := ""
	if pack.NewsContext != nil {
...
func buildStrategyGenerationPrompt(taskID string, genCtx StrategyGenerationContext, mcpURL string) string {
	...
	b.WriteString("Every draft, including strategy_patch and no_change, must include exactly three horizon_outlooks: short/5 trading days, medium/20, and long/60. Direction must be exactly bullish, neutral, or bearish; never invent hybrid labels. Every numeric field is mandatory and must never be null or omitted: asOfPrice, probabilityUp, probabilityOutperform, expectedPrice, expectedReturnPct, rangeLow, rangeHigh, targetPrice, targetProbability, downsideRiskPct, and confidence. Each item also requires a non-empty thesis, at least one invalidConditions entry, an uncertainties array, and dataQuality. Even when dataQuality is degraded or insufficient, provide the model's best conditional numeric estimate and explain uncertainty instead of omitting fields. Use the opportunity candidate's supplied outlooks as evidence when present, but independently revise them when fresher verified facts justify a change.\n")
...
func buildStrategyGenerationStepPrompt(taskID string, pack StrategyGenerationStepPack, mcpURL string) string {
	...
	if pack.StepKey == StrategyGenerationStepFormatter {
		...
		b.WriteString("Every rule.action must be exactly one of observe, build_position, add_position, hold, reduce_position, or exit_position. Quote refresh, report review, filing monitoring, order verification, waiting, and reassessment are all action observe with their meaning in title/trigger; never invent a custom action enum.\n")
