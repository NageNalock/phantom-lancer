...
func TestBuildPortfolioSentinelPromptRequiresCompleteNewsContextReview(t *testing.T) {
	...
	for _, want := range []string{
		...
		"A daily-bar summary is not a substitute for raw OHLCV",
		"use at most 4 discretionary MCP retrieval calls in total",
		"stock_agent.get_daily_bars",
		"potentialSupplyPressure",
		"possible supply/distribution risk",
		"defensive reduce/exit plan may instead rely on fresh internal quote/bar/flow evidence",
