...
func (s *Server) RegisterStockV2Routes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/stockv2/assets/readiness/evaluate", s.handleStockV2EvaluateAssetReadiness)
	mux.HandleFunc("GET /api/stockv2/assets/readiness/overview", s.handleStockV2GetAssetReadinessOverview)
...
func (s *Server) handleListInstruments(w http.ResponseWriter, r *http.Request) {
	...
	profileStatus := r.URL.Query().Get("profileStatus")
	...
	instruments, err := s.stockV2.GetInstrumentsFiltered(ctx, market, instrumentType, profileStatus, limit, offset)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	total, err := s.stockV2.CountInstrumentsFiltered(ctx, market, instrumentType, profileStatus)
	if err != nil {
		// 计数失败不影响主数据，返回 0 即可
		total = 0
	}
...
func (s *Server) handleSearchInstruments(w http.ResponseWriter, r *http.Request) {
	...
	instruments, err := s.stockV2.SearchInstrumentsFiltered(ctx, keyword, market, instrumentType, profileStatus, limit)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
