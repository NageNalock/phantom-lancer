...
func buildStockSetupGuide() string {
	...
	w("# ========== 目标域名白名单 ==========")
	w("# 东财行情 / 主数据")
	w("acl allowed_domains dstdomain .eastmoney.com")
	w("acl allowed_domains dstdomain push2.eastmoney.com")
	w("acl allowed_domains dstdomain push2his.eastmoney.com")
	w("acl allowed_domains dstdomain datacenter-web.eastmoney.com")
	w()
	w("# 新浪行情 / 主数据")
	...
	w()
	w(bq3, "bash")
	w("# 1) 东财 - 应该返回 HTTP 200 + JSON")
	w(`curl -x http://PROXY_IP:31280 \`)
	w(`  -s -o /tmp/em.json -w "eastmoney: http=%{http_code} size=%{size_download}\n" \`)
	w(`  -H 'Referer: https://quote.eastmoney.com/' \`)
	w(`  -H 'User-Agent: Mozilla/5.0' \`)
	w(`  'https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=2&po=1&np=1&fltt=2&invt=2&fid=f3&fs=m:0+t:6&fields=f12,f14'`)
	w()
	w("# 2) 新浪 - 应该返回 HTTP 200 + JSON")
	w(`curl -x http://PROXY_IP:31280 \`)
