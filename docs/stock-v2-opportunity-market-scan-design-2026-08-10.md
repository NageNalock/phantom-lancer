...
行业或上级板块至少一项有值才算分类完整；完整率低于 95% 时整轮在本地预筛阶段失败并保留健康快照，不允许把大量“未知行业”误判成没有轮动。每个涌现或确认板块最多选择 4 个代表股，代表股评分会抑制已经极端上涨的成员，避免席位全部被最热龙头占据。最多 6 个板块、24 个代表股获得板块准入，Agent 研究池为其中最多 8 个保留席位。
