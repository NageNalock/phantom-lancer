...
全市场数据维护成功
  ...
  → 股票画像补齐行业、概念和业务关键词
  → 行业/板块横截面聚合与连续状态判断
  → 行情 160 + 板块代表 24 + 最近实质变化消息 16（去重回填后最多 200）
  → 前复权日线和最新报价补齐（最多 60）
...
本地预筛采用趋势、动量、量能、上涨成交占比、流动性、行业广度和 120 日相对位置的百分位组合。个股排序之前，系统同时按行业和上级板块聚合成员站上 MA20 的比例及 3 日变化、5 日上涨广度、3/5/20 日中位收益、放量成员比例和前 200 名渗透率，形成 `emerging / confirmed / overheated / fading / invalidated` 状态。状态按历史扫描快照延续首次发现交易日和连续天数；它只决定研究优先级，不绕过确定性决策门。最终排序加入资金流、最新行情、消息脉络、数据质量和短期过热惩罚。分数用于缩小研究集合，不等同于投资结论。

行业或上级板块至少一项有值才算分类完整；完整率达到 95% 为健康，80% 至 95% 降级运行并在前端和 Agent 上下文高亮缺口，低于 80% 时整轮在本地预筛阶段失败并保留健康快照，不允许把大量“未知行业”误判成没有轮动。每个涌现或确认板块最多选择 4 个代表股，代表股评分会抑制已经极端上涨的成员，避免席位全部被最热龙头占据。最多 6 个板块、24 个代表股获得板块准入，Agent 研究池为其中最多 8 个保留席位。
...
消息召回只读取最近 72 小时、置信度不低于 0.40、阶段为 emerging / spreading / accelerating / restarting 的实质变化主题。它依次使用明确股票代码或名称、股票画像结构字段、画像关键词和现有向量资产召回主板标的。最多 16 个消息候选可以越过行情基础 160 名门槛，最多 6 个保留 Agent 复核席位，总预算仍为 200 / 20。纯语义召回不增加主题分，只获得研究席位并标记为“待因果核验”。
...
| --- | ---: |
| 本地预筛 | 200 |
| 行情基础准入 | 160（包含在本地预筛 200 内） |
| 板块代表准入保留 | 24（最多 6 个板块，每个最多 4 只） |
| 板块候选复核保留 | 8（包含在 Agent 复核 20 内） |
| 前复权与最新报价 | 60 |
| 120 日资金流 | 30 |
| Agent 证据复核 | 20 |
| 最终候选 | 10 |
| 策略草案 | 10 |
| 消息候选准入保留 | 16（包含在本地预筛 200 内） |
...
- `mode=market_scan`；
- `marketScanRunId`；
- `sectorSnapshot`：分类覆盖率、板块状态、首次发现日期、连续天数、扩散/收益/量能指标和代表股；
- 服务端预筛的最多 20 个 `marketCandidates`。
- 每个候选的 `admissionReasons` 和 `sectorSignals`；
- 每个消息候选的 `threadId / versionId / matchKind / matchedTerms / semanticScore / requiresCausalVerification`。

模型先判断板块处于早期扩散、持续确认、交易过热还是退潮，再比较同板块代表股；只能从有界个股集合返回候选，后端会再次验证子集关系和最多 10 个的输出上限。模型必须通过 `stock_agent.get_news_thread` 读取每个引用的准确主题版本；画像、语义或板块命中都不是买入证据，必须继续核验真实业务暴露、事件传导路径、价格是否已反映和失效条件。建议绑定 `GPT-5.6-Terra / Codex CLI / medium`，但实现不会覆盖 owner 已保存的模型、执行模式或留空的推理强度。
...
- `stockv2_decision_fund_flow_cache`（组合哨兵当前股票持仓的最新完整交易日资金证据）
...
结果页用可折叠紧凑表展示每轮板块状态、首次发现日期、连续天数、站上 MA20 比例、3 日扩散、5 日中位收益、放量占比和代表股；旧历史 run 明确提示尚未启用板块检测。候选列表以单行省略形式展示筛选结论、四道门通过数、数据健康和“行情预筛 / 板块轮动 / 消息驱动 / 多路准入”来源，并支持按板块轮动或消息相关过滤。详情抽屉展示候选的板块准入信号，以及主题版本、匹配方式、命中词、语义分数和待因果核验状态。固定 5 日 18% / 20 日 35% 边界已由 ATR 动态波动门替代；详情同时展示催化定价、因子拥挤、事件保护、来源健康与 1/3/5/10/20 交易日事后验证。历史记录从已保存的运行快照、发现报告与策略决策账本按需恢复，不重新运行模型，也不伪装成已经通过新门。
