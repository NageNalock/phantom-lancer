...
后台监控不是用户手动创建、编辑、删除的独立价格规则对象，而是系统固化的任务行为。用户可以暂停某类监控、调整周期、调整作用范围和 Agent 预算，但不应被要求逐个创建“单票 + 价格规则”的监控任务。
...
数据面策略监控读取策略操作剧本里的 `dataPrefilters` 和 `portfolioPrefilters`，命中后生成带有 `matchedAction` 的候选命中，例如建仓、加仓、减仓或清仓候选。旧的单一触发价字段不再作为监控输入。
