...
`NewsLinkCandidate` 是候选关系，不是事实关系。为了避免漏掉关键信息，第一轮过滤应偏向高召回：阈值可以保守偏低，保留 topK 候选；显式命中的股票名、代码、持仓股票和活跃策略可以降低进入门槛。

候选消息由消息脉络、组合哨兵和机会发现按各自窗口读取。需要 Agent 判断时，系统构造 `Agent Context Pack`，把消息、候选关联、实时行情、历史 K 线摘要与有界近期明细、量价结构特征、组合快照、策略、内部预筛结果、数据新鲜度和近期同类记录一起交给 Agent。消息候选本身不再启动独立的逐条监控任务。

StockV2 MCP 的只读数据面按用途拆成有界工具：标的与画像、最新行情、日线明细、分钟线、盘中报价历史、日级资金流、决策门/财务/事件事实、市场扫描候选、新闻与消息脉络、策略及组合上下文。工具只返回业务证据，不返回数据源凭据、代理或私有配置；每个序列接口必须有服务端条数和时间范围上限。上下文摘要不能替代原始序列：当确定性门提示放量滞涨、突破后承接弱、低位收盘或价格—资金背离时，Agent 应按需读取对应明细复核。
...
标的主数据写入时同步更新对应的确定性基础画像。自动画像维护只修复缺失画像，再按配置的小批量队列做深度更新；不得周期性遍历并重建全部已有画像。owner 显式发起的全量重建仍保留，用于人工修复或结构迁移。
...
前端完整的嵌入模型绑定、向量维护、资产统计和明细归入“Agent → 语义召回”。主题研究等业务页面只展示紧凑的可用性状态和管理入口，不把基础设施管理面板常驻在业务工作区。
...
命令只启动 StockV2 存储和向量客户端，不启动 HTTP、调度器或 Agent。模型切换会把旧模型资产写入持久向量工作队列，命令按队列逐批写入；进程中断后重复执行同一命令即可从未完成工作项续跑。迁移完成必须逐源验证模型、维度、文本哈希、资产状态和 DuckDB `vector_ref`；验证通过前不得删除旧向量或启动正式服务。验证通过后命令删除所有非目标模型的 SQLite 资产和 DuckDB 向量、执行 checkpoint，并恢复目标模型的自动维护。迁移失败时保持自动维护关闭，避免部分索引被正式服务继续使用。

## 5. 后台监控与命中

后台监控是系统内置能力，不是用户逐条创建的业务对象。

通用监控任务只保留已有完整执行链路的两类：

- 最新行情刷新：维护持仓、策略和关注标的的最新可交易价格与新鲜度状态。
- 数据面策略监控：扫描行情、日 K、成交量、数据新鲜度与策略条件的匹配情况。

用户配置的是任务开关、周期、作用范围、敏感度、冷却时间和 Agent doublecheck 是否启用，而不是逐个配置单票价格突破规则。

每次后台任务执行都应生成 `MonitorRun`，记录任务类型、扫描范围、输入数据窗口、扫描数量、命中数量、耗时、状态和失败原因。命中策略、组合、股票或消息时生成 `MonitorHit`，记录命中的对象、证据、预筛原因和后续处理结果。

`MatchRule` 是内部预筛能力，不是用户主模型。价格、涨跌幅、日 K、数据新鲜度和组合权重等规则都只是为了发现值得进入 Agent doublecheck 的候选命中。

报价过期和单票仓位上限不再由一项独立周期任务重复扫描：需要持续观察时由 Watch 规则表达，进入操作链路时由 guardrails 再做确定性拦截。窗口级的持仓消息与风险研判由独立的组合哨兵负责；它有自己的配置、调度和历史，不注册成通用监控任务。

数据面策略监控应优先读取策略操作剧本里的 `dataPrefilters` 和 `portfolioPrefilters`，命中后生成带有 `matchedAction` 的候选命中，例如建仓、加仓、减仓或清仓候选。旧的单一触发价字段只能作为兼容兜底。

价格证据必须区分用途：前复权收盘只用于趋势、收益和 ATR 计算；可执行价格条件使用带时点的最新原始报价，日收盘条件使用未复权的已完成日线。三者不得复用同一个 `latestPrice` 字段。

组合哨兵生成的条件计划只观察计划生效后的新数据。价格和涨跌幅条件要求生效后的真实阈值穿越，日收盘条件要求生效后形成的新收盘；计划生成前已满足的历史状态不能创建操作提案。前端必须把“当前建议”和“条件风控动作”分开展示。

新的组合哨兵结论不会仅因本轮返回 `hold` 就抹掉仍在有效期内、尚未触发的保护条件。发布新策略版本时，服务端按组合、标的和条件类型去重：本轮同类条件替换旧条件，不同类型的有效保护条件继续由数据面策略监控观察；已触发、过期或无可执行条件的旧规则不继承。

组合哨兵构造决策上下文前，会使用市场扫描已经配置的主备资金流源，对当前股票持仓执行一次有界的定向预取。已覆盖最新完整交易日的市场扫描证据或持仓缓存会直接复用，不因为持仓未进入当日市场扫描候选而统一标记为缺失；场内基金不请求个股 `moneyflow`，数据健康状态明确显示为“不适用”。资金流仍是可选交叉证据，预取失败会降级显示但不阻断持仓风险研判。

最新行情刷新可以在持仓标的首次跨越盘中显著涨跌边界时，按固定冷却窗口触发一次组合哨兵趋势重评。该触发只启动研判，不直接创建操作；全市场标的不进入 60 秒轮询，仍由每日市场扫描和消息主题变化发现。
...
每条操作动作规则可以包含触发描述、前置条件、目标状态、风险备注，以及供后台监控预筛使用的 `dataPrefilters` 和 `portfolioPrefilters`。
