...
- 所有路径必须经过后端规范化。
- 只允许访问配置的根目录下的应用。
- 新增应用目录必须等于允许根目录，或位于允许根目录下；目录不存在时，只有用户明确选择创建，后端才可以在允许根目录内创建。
...
- 项目路径必须落在允许根目录内。
- Codex 会话可以选择无项目，此时不绑定工作目录，默认只读。
- 绑定项目的 Codex 会话可选择 `read-only` 或 `workspace-write`。
- `workspace-write` 必须由项目显式开启。
