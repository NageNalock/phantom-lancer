...
## 2026-08-18 实现更新：模型周期预期 v3

- 新运行固定使用 `portfolio-sentinel-report/v3`；历史 v1/v2 结果继续只读展示，不补造预测值。
- 每个持仓动作计划必须同时返回 short/5、medium/20、long/60 三个模型价格预期；每个被检查组合也必须返回三周期组合收益与最大回撤预期。
- 概率、预期价、目标触及概率和价格区间由模型综合上下文分析生成。服务端只做 schema、数值范围、三周期完整性和区间一致性校验，不运行确定性概率预测引擎。
- 模型预测与动作置信度分开；数据缺失或互相冲突时仍可给出条件判断，但必须降低模型置信度并标记 `degraded` 或 `insufficient`。
- 组合哨兵仍只形成待确认计划，不执行交易。三周期预测用于解释和协调计划，真正的条件命中仍由现有确定性监控处理。

## 2026-08-31 实现更新：机会交接与执行边界

- 全市场发现仍由市场扫描、主题研究和策略生成负责；组合哨兵不重复承担全市场研究。
- 组合哨兵只接收最多 4 个近期可信候选作为可选建仓交接。普通机会信号只保留 96 小时，已激活单票策略不受该时限影响；选择顺序为已激活策略、已生成策略、待生成策略、入围候选，同标的去重。
- 模型必须完整复核全部持仓；对可信候选先使用服务端已有证据，仅当候选足以形成建仓计划时才做额外公开验证。该边界避免历史候选挤占持仓风险研判的执行时限。
- Prompt 使用字段级压缩而不是字节截断：每个持仓的报价、日线、分钟趋势和确定性闸门必须完整保留；完整画像正文、重复的全局新闻和旧 Review 大对象不进入本轮 Prompt。压缩结果始终是有效 JSON，外部检索和自由 MCP 检索分别最多 4 次。
- `list_news_context_changes` 与 `list_portfolio_sentinel_impact_review_scope` 只在消息脉络最终影响复核中暴露；普通手动/定时组合哨兵禁用这两个工具，避免工具描述诱导模型执行不存在的五类冻结范围。组合哨兵统一使用 20 分钟硬超时，超时不会保存半成品计划。
