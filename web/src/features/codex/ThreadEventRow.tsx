...
export function EventRow({ event }: { event: CodexEvent }) {
  ...
  if ((isCommand || isFileChange || isStructured) && event.textPreview) {
    return (
      <details className={`rounded-md border px-3 py-2 text-sm ${rowClass(tone, false)}`}>
  ...
  return (
    <div className={`rounded-md border px-3 py-2 text-sm ${rowClass(tone, isUser)}`}>
...
function rowClass(tone: string, isUser: boolean): string {
