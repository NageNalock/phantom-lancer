...
export function ObjectStoragePanel({ actions }: { actions: AppActions }) {
  ...
  async function remove(profile: ObjectStorageProfile) {
    if (!window.confirm(`确认删除对象存储 profile「${profile.name || profile.id}」？该操作不可恢复，使用它的历史资产将无法读取。`)) return;
      ...
      <div className="grid gap-3">
        <div className="grid content-start gap-2">
          {items.length ? (
            ...
            <EmptyState body={loading ? "正在加载 profile。" : "尚未配置对象存储 profile，使用右侧表单添加。"} title="暂无 profile" />
          )}
        </div>
