        ...
        <div className="grid gap-4">
          ...
          <CollapsibleSection title="高级设置" subtitle="固定周期、执行安全策略、下次运行和依赖能力只读展示">
            <div className="grid gap-0 rounded-lg border border-[var(--line)] bg-[var(--surface)]">
              ...
              <ReadOnlyRow label="窗口单次尝试上限" value={formatNewsContextInterval(config.agentTimeoutSeconds || 1800)} detail="同时约束四小时与日级窗口的整次尝试；超时会终止模型请求或进程组并保留未完成清单" />
              <ReadOnlyRow label="窗口与复核自动重试" value={`各 ${config.timeoutRetryLimit ?? 2} 次`} detail={`窗口尝试与后续影响复核分别计数；首次等待 ${formatNewsContextInterval(config.retryBackoffSeconds || 60)}，随后指数退避，耗尽后保留最终失败`} />
              <ReadOnlyRow label="影响复核单次上限" value={formatNewsContextInterval(config.reviewTimeoutSeconds || 600)} detail="四小时和日级结论触发的组合影响复核使用同一有限重试策略" />
