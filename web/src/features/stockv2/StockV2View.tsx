import { ArrowClockwise, ChartLine, Crosshair, Database, Eye, Faders, Plus, Wallet } from "@phosphor-icons/react";
...
import { StockV2Watches } from "./StockV2Watches";
...
export function StockV2View({ actions, data }: { actions: AppActions; data: AppData }) {
  const [activeTab, setActiveTab, tabHref] = useQueryParamState<StockV2Tab>(
    "stockv2",
    ["overview", "universe", "dailyBars", "portfolios", "strategies", "watches", "settings"],
    "overview",
  );
  ...
  return (
    <div className="grid min-h-[calc(100dvh-104px)] grid-cols-[minmax(0,1fr)_340px] max-xl:grid-cols-1">
      <div className="grid content-start gap-4 p-5">
        ...
        {activeTab === "watches" ? <StockV2Watches actions={actions} data={data} /> : null}
