...
export function StockV2OpportunityMarketScan({ actions }: { actions: AppActions }) {
  ...
  async function loadCandidates(runId: string) {
    ...
    try {
      const result = await actions.api<{ items: StockV2OpportunityMarketScanCandidate[] }>(
        `/api/stockv2/opportunity-market-scan/runs/${encodeURIComponent(runId)}/candidates?limit=200`,
      );
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { void load(); void loadCandidates(selectedRunId); }, 15_000);
    return () => window.clearInterval(timer);
  }, [selectedRunId, status?.activeRun?.id]);
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid grid-cols-[280px_minmax(0,1fr)] gap-4 max-xl:grid-cols-1">
        ...
        <Panel
          title={selectedRun ? `${selectedRun.tradeDate || "扫描"} · 候选` : "候选"}
        ...
        >
          ...
          {candidates.length === 0 ? <EmptyState title="暂无候选" body={selectedRun && ACTIVE_STATUSES.has(selectedRun.status) ? "扫描仍在执行，候选会随阶段持续落盘。" : "本轮没有形成候选记录。"} /> : (
...
function CandidateRow({ candidate, onSelect }: { candidate: StockV2OpportunityMarketScanCandidate; onSelect: () => void }) {
  return <tr className="border-b border-[var(--line)] hover:bg-[var(--surface-soft)]">
    ...
    <td className="px-2 py-2 font-mono">{candidate.metrics.fundFlowAvailable ? candidate.flowScore.toFixed(0) : "缺失"}</td>
...
function MarketScanSettings({ actions, status, onClose, onSaved }: { actions: AppActions; status: StockV2OpportunityMarketScanStatus; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function save() {
    setSaving(true);
    try {
      await actions.api("/api/stockv2/opportunity-market-scan/config", { method: "PATCH", body: JSON.stringify({ enabled }) });
...
function CandidateDrawer({ actions, candidate, discoveryRunId, onClose }: { actions: AppActions; candidate: StockV2OpportunityMarketScanCandidate; discoveryRunId?: string; onClose: () => void }) {
  ...
  return <Drawer title={`${candidate.name} · ${candidate.symbol}`} subtitle={`综合排名 ${candidate.finalRank || candidate.prefilterRank || "-"} · ${candidate.industry || "行业未标注"}`} onClose={onClose} width={620}>
    <div className="grid gap-4 text-sm">
      <div className="grid grid-cols-3 gap-2"><ScanMetric label="综合评分" value={candidate.finalScore.toFixed(1)} /><ScanMetric label="资金评分" value={candidate.flowScore.toFixed(1)} /><ScanMetric label="主题评分" value={candidate.themeScore.toFixed(1)} /></div>
      <Panel title="行情与质量"><dl className="grid grid-cols-2 gap-3 text-xs"><MetricItem label="最新价" value={metrics.latestPrice?.toFixed(3) || "-"} /><MetricItem label="当日涨跌" value={formatPct(metrics.latestPctChange)} /><MetricItem label="5 日收益" value={formatPct(metrics.return5Pct)} /><MetricItem label="20 日收益" value={formatPct(metrics.return20Pct)} /><MetricItem label="量比 5/20" value={metrics.volumeRatio5To20?.toFixed(2) || "-"} /><MetricItem label="资金流" value={metrics.fundFlowAvailable ? `20日均占比 ${metrics.mainFlowRatio20?.toFixed(2)}%` : "未取得"} /></dl></Panel>
...
function candidateStageLabel(stage?: string) { return ({ prefiltered: "预筛", research_candidate: "待复核", final: "最终候选", excluded: "已排除" } as Record<string, string>)[stage || ""] || stage || "-"; }
