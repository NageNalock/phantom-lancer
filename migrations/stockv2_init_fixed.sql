...
CREATE INDEX IF NOT EXISTS idx_stockv2_portfolios_risk_level ON stockv2_portfolios(risk_level);
...
CREATE INDEX IF NOT EXISTS idx_stockv2_holdings_market ON stockv2_holdings(market);

CREATE INDEX IF NOT EXISTS idx_stockv2_update_jobs_status ON stockv2_update_jobs(status);
CREATE INDEX IF NOT EXISTS idx_stockv2_update_jobs_trigger_type ON stockv2_update_jobs(trigger_type);
...
BEGIN
    INSERT INTO stockv2_update_progress (
        update_job_id, processed_count, success_count, current_batch, current_batch_progress, updated_at
    ) VALUES (NEW.id, 0, 0, 0, 0, datetime('now'));
END;
